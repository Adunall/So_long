...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/09 19:34:58 by adunal            #+#    #+#             */
/*   Updated: 2023/06/23 14:31:27 by adunal           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	size_control(t_data *data)
{
	...
	while (1)
	{
		...
		if (x != ft_strlen(line) && line[0] != '\0')
			err_msg("Error\n-->Map is not rectangular, pls fix it.", data);
	}
	if (!line)
		free(line);
	data->map_height = y;
...
void	wall_control(t_data *data)
{
	...
	while (x < data->map_width)
		if (data->map[0][x++] != '1')
			err_msg("Error\n-->1 The map has to be surrounded by walls!!", data);
